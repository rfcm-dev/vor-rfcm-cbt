            ...
            <form onSubmit={(e) => { e.preventDefault(); lookupStudent(); }} className="space-y-5">
              ...
              {notRegistered && (
                <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 text-center">
                  <p className="text-sm text-amber-800 mb-3">You are not registered yet. Please register first before taking the exam.</p>
                  <a href="/register" className="inline-block rounded-xl bg-rfcm-red text-white text-sm font-semibold px-5 py-2.5 hover:bg-rfcm-red-dark transition-colors">
                    Go to Registration
                  </a>
                </div>
              )}
              <div className="flex gap-3 pt-2">
                ...
                <PrimaryButton disabled={loading || !studentName.trim() || notRegistered} type="submit" className="flex-1">
                  {loading ? "Checking..." : "Continue"}
                </PrimaryButton>
              </div>
            </form>
        ...
        {step === "profile" && selectedStudent && (
          <div className="text-center space-y-5">
            ...
            <div className="flex flex-col items-center gap-4">
              <div className="relative">
                {selectedStudent.photo_url && !photoError ? (
                  <img src={selectedStudent.photo_url} alt={selectedStudent.name} onError={() => setPhotoError(true)} className="w-24 h-24 rounded-full object-cover border-4 border-white shadow-lg" />
                ) : (
